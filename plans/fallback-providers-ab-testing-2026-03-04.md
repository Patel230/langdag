...
- [x] 3b: Add `vertex.go` — Vertex AI Gemini variant (different base URL, Google OAuth instead of API key)
- [x] 3c: Tests for Vertex Gemini variant
...
- [x] 6b: Log routing decisions — log which provider was selected, whether fallback was triggered, and which provider ultimately succeeded
...
- [x] 7b: Add example config in `examples/` — sample `config.yaml` showing multi-provider routing, fallback, and per-provider retry setup
