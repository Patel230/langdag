// Package gemma provides Google Gemma provider implementation.
// Gemma models are served through the same Google AI Studio endpoint as
// Gemini, so the wire protocol is identical to gemini's; this file is a
// parallel of internal/provider/gemini/protocol.go intentionally kept
// separate so the two providers can diverge as needed.
package gemma
...
// gemmaTool represents a single entry in the tools array.
...
type gemmaTool struct {
	...
	// For server-side tools — the tool name (e.g. "google_search")
	serverToolName string
}

// MarshalJSON implements custom marshaling. Server tools are serialized as
// {"<name>": {}} which is the Google AI Studio format for built-in tools.
func (t gemmaTool) MarshalJSON() ([]byte, error) {
	if t.serverToolName != "" {
...
// gemmaServerTools maps standardized tool names to provider-specific names.
var gemmaServerTools = map[string]string{
	types.ServerToolWebSearch: "google_search",
}

func convertTools(tools []types.ToolDefinition) []gemmaTool {
	var decls []functionDeclaration
	var serverTools []gemmaTool
	...
	for _, t := range tools {
		...
		toolName := t.Name
		if mapped, ok := gemmaServerTools[t.Name]; ok {
			toolName = mapped
		}
		serverTools = append(serverTools, gemmaTool{serverToolName: toolName})
	}

	var result []gemmaTool
	if len(decls) > 0 {
		result = append(result, gemmaTool{FunctionDeclarations: decls})
	}
...
func convertResponse(resp *gemmaResponse) *types.CompletionResponse {
...
func doHTTPRequest(ctx context.Context, client *http.Client, url string, body []byte, headers map[string]string) (io.ReadCloser, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("gemma: failed to create request: %w", err)
	}
